...
int main(int argc, char** argv) 
{
	...
	for(int y= 0; y<fbvar.yres_virtual; y++)
	{	for(int x = 0; x<fbvar.xres_virtual/3; x++) {
			int offset = (x+y*fbvar.xres_virtual) * NO_OF_COLOR;
	...
	for(int y = 0; y<fbvar.yres_virtual; y++)
	{	for(int x = fbvar.xres_virtual/3; x<2*(fbvar.xres_virtual/3); x++) {
			int offset = (x+y*fbvar.xres_virtual) * NO_OF_COLOR;
			lseek(fd, offset, SEEK_SET);
			pixel = makepixel(255,255,255);
			write(fd, &pixel, NO_OF_COLOR);
	...
	for(int y = 0; y<fbvar.yres_virtual; y++)
	{	for(int x = 2*(fbvar.xres_virtual/3); x<fbvar.xres_virtual; x++) {
			int offset = (x+y*fbvar.xres_virtual) * NO_OF_COLOR;
			lseek(fd, offset, SEEK_SET);
